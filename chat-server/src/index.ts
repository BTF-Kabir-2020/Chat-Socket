  ...
  transports: [
    new winston.transports.Console({
      format: winston.format.combine(
        winston.format.colorize(), // رنگی کردن لاگ‌ها برای دید بهتر
...
const usersInRoom: { [key: string]: { [socketId: string]: string } } = {};
const lastActivityTime: { [key: string]: number } = {}; // زمان آخرین فعالیت هر کاربر
...
app.post(
  ...
  (req: Request, res: Response): void => {
    ...
    const senderName: string = req.body.senderName; // دریافت نام فرستنده از بدنه درخواست
    if (!file) {
...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("join_room", (room: string, name: string) => {
